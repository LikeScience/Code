...
int main() {
    ...
    F(i,0,t) sort(v.begin()+i*k, v.begin()+(i+1)*k);
    F(i,0,t) {
    ...
    F(i,0,rep) {
        ...
        int f = d;
        while(d==f) f = rd()%t;
        int i1 = rd()%k, i2 = (lower_bound(v.begin(), v.end(), v[k*d+i1])-v.begin())%k;
    ...
    ot << bd << '\n';
    ot << n << " " << k << '\n';
    F(i,0,n) ot << bv[i].se << "," << bv[i].fi << "\n";
    //cout << m.begin()->fi << " " << (--m.end())->fi << " " << (--m.end())->fi-m.begin()->fi << '\n';
    F(i,0,n) cout << bv[i].se << " \n"[i==n-1];
}